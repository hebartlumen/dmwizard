...
class WPSEO_Bulk_List_Table extends WP_List_Table {
	...
	function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			...
			if ( is_array( $post_types ) && $post_types !== array() ) {
				...
				printf( '<select name="post_type_filter" id="post-type-filter-%2$s">%1$s</select>', $options, esc_attr( $instance_type ) );
	...
	protected function parse_meta_data_field( $record_id, $attributes, $values = false ) {
		...
		return sprintf( '<td %2$s id="wpseo-existing-%4$s-%3$s">%1$s</td>', esc_html($meta_value), $attributes, esc_attr($record_id), $this->target_db_field );
	}
